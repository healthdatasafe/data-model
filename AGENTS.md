...
**Plan 71 (v1.10.0) — questionnaire request/answer event pair.** Plan 71 adds two eventTypes to support explicit yes / no / unknown / declined patient answers without forking the canonical clinical-domain vocabulary: `questionnaire/request-v1` (doctor → patient — questions map keyed by stable [a-zA-Z0-9_-]+ keys, each referencing an existing HDS item via `itemRef` with a temporal `scope` and optional `subField` qualifier) and `questionnaire/answer-v1` (patient → doctor — per-question `status` enum with `oneOf` discriminator). Canonical clinical records remain in the typed eventTypes (`medication/coded-v1`, `body-weight/kg`, `fertility-cycles-start`, …) — the answer event references them. Documented at [`documentation/QUESTIONNAIRE.md`](documentation/QUESTIONNAIRE.md); cross-reference convention `clientData.related.<eventId>: true` (Pryv §7 keyed-object) makes cohort queries direct. **No per-domain assertion eventTypes** (`medication/exposure-assertion-v1` was rejected) — the explicit-no / unknown / declined semantics live entirely on the answer event; FHIR `MedicationStatement.status=not-taken` exports are derived at the consumer. Do not invent new per-domain assertion eventTypes; if the rare case calls for it (a clinical record needing standalone existence outside any questionnaire), bring the design back to a plan first.
